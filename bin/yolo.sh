...
if [ ! -z "$YOLO_PHP" ] ; then
  # Use the YOLO_PHP environment variable if it is available.
  php="$YOLO_PHP"
else
...
# Pass in the path to php so that yolo knows which one
# to use if it re-launches itself to run other commands.
export YOLO_PHP_USED="$php"

exec "$php" $YOLO_PHP_ARGS "$SCRIPT_PATH" "$@"
